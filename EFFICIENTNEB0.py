...
num_cancerous_images = len(cancer_labels)
num_non_cancerous_images = len(non_cancer_labels)
...
num_samples_cancerous = min(num_samples // 2, len(cancerous_indices))
...
X_cancer_train, X_cancer_test, y_cancer_train, y_cancer_test = train_test_split(cancer_images, cancer_labels, test_size=0.2, random_state=42)
...
# Load pre-trained EfficientNet model without the top classification layer
efficientnet_model = EfficientNetB0(include_top=False, input_shape=(image_height, image_width, 3), weights='imagenet')

# Freeze the layers of the EfficientNet model
for layer in efficientnet_model.layers:
    layer.trainable = False

# Create a new model and add the EfficientNet model as a layer
model_efficientnet = Sequential()
model_efficientnet.add(efficientnet_model)
model_efficientnet.add(Flatten())  # Add a flatten layer to convert feature maps to a 1D feature vector
model_efficientnet.add(Dropout(0.5))  # Add a dropout layer with rate 0.5
model_efficientnet.add(Dense(num_classes, activation='softmax', kernel_regularizer=regularizers.l2(0.01)))  # Add a fully connected layer with softmax activation
...
# Train the model
history_efficientnet = model_efficientnet.fit(augmented_train_data, epochs=5, validation_data=(X_test, y_test_encoded), callbacks=[early_stopping])

# Evaluate the model on the test set
loss, accuracy = model_efficientnet.evaluate(X_test, y_test_encoded)
